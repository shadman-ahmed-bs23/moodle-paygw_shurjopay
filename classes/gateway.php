...
class gateway extends \core_payment\gateway {
    ...
    public static function add_configuration_to_gateway_form(account_gateway $form): void {
        ...
        $mform->addElement('text', 'username', get_string('username', 'paygw_shurjopay'));
        $mform->setType('username', PARAM_TEXT);
        $mform->addHelpButton('username', 'username', 'paygw_shurjopay');

        $mform->addElement('text', 'password', get_string('password', 'paygw_shurjopay'));
        $mform->setType('password', PARAM_TEXT);
        $mform->addHelpButton('password', 'password', 'paygw_shurjopay');

        $paymentmodes = array(
            'sandbox' => get_string('paymentmodes:sandbox', 'paygw_shurjopay'),
            'live' => get_string('paymentmodes:live', 'paygw_shurjopay'),
        );
        $mform->addElement('select', 'paymentmodes', get_string('paymentmodes', 'paygw_shurjopay'), $paymentmodes);
